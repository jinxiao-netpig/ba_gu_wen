...
1. select 语句中除了 default 之外，每个 case 操作⼀个 channel，要么读要么写
2. 除 default 之外，各个 case 执⾏顺序是随机的

## 1、为什么 select 在多个 case 未阻塞时要随机选一个及其机制？

**原因**：

- 提高 IO 复用
- 避免饥饿问题，即某个 case 始终被优先选择，导致其它 case 无法执行。通过随机选择，每个 case 有相等的机会被执行，从而平衡了程序的执行
