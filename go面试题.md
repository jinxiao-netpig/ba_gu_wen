...
![img](https://raw.githubusercontent.com/jinxiao-netpig/user-image/master/imgs/202410081701956.png)

以T2结构体为例，实际存储数据的只有13字节，但实际用了24字节，浪费了11个字节：

![img](https://raw.githubusercontent.com/jinxiao-netpig/user-image/master/imgs/202410081701160.png)

以T3结构体为例，实际存储数据的只有13字节，但实际用了16字节，浪费了3个字节：

![img](https://raw.githubusercontent.com/jinxiao-netpig/user-image/master/imgs/202410081702555.png)
...
![img](https://raw.githubusercontent.com/jinxiao-netpig/user-image/master/imgs/202410081711388.png)

## 3、Go GC算法——三色标记法

此算法是在Go 1.5版本开始使用，Go 语言采用的是标记清除算法，并在此基础上使用了**三色标记法**和**混合写屏障技术**，GC过程和其他用户goroutine可并发运行，但**需要一定时间的STW**。
...
对象被删除时触发的机制（只在堆内存中生效）：赋值器将这一行为通知给并发执行的回收器，被删除的对象，如果自身为灰色或者白色，那么标记为**灰色**
...
![img](https://raw.githubusercontent.com/jinxiao-netpig/user-image/master/imgs/202410081723773.png)
...
![img](https://raw.githubusercontent.com/jinxiao-netpig/user-image/master/imgs/202410091654796.png)
